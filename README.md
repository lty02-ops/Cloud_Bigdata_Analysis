# Cloud_Bigdata_analysis


## 1. 문제 정의

최근 Kubernetes, Docker, Terraform 등 클라우드 및 클라우드 네이티브 기술의 사용이 증가하고 있다.

해당 프로젝트에서는 GitHub 공개 이벤트 데이터에서 Repository 이름에 포함된 클라우드 네이티브 기술 키워드를 추출 및 분석하고, 기술 간 조합을 분석하여 실제 환경에서 자주 사용되는 기술 스택 조합을 파악하고자 한다.

또한 Hadoop 기반 빅데이터 처리 환경을 이용하여 대규모 데이터를 저장 - 처리 - 분석 하는 파이프라인을 구현하고자 한다.

<br>

## 2. 기술 스택

데이터 수집 - Python

<br>

데이터 저장 - HDFS

<br>

데이터 처리 - Apache Spark

<br>

데이터 분석 - Hive / Spark SQL

<br>

시각화 - Matplotlib

<br>

실행 환경 - HDP Sandbox

<br>

## 3. 구현 계획

1. GitHub 공개 이벤트 데이터 수집
2. HDFS에 데이터 저장
3. Hive를 이용한 데이터 테이블 생성
4. Spark 기반 데이터 전처리 및 분석
5. Repository 이름에서 클라우드 네이티브 기술 키워드 추출
6. 기술별 활동량 분석
7. 기술 조합 생성 및 빈도 분석
8. 조합 크기별 상위 기술 조합(Top 5) 추출
9. 결과 시각화 및 분석
